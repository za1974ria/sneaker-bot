...
def _build_hit(
    ...
    *,
    ...
    precision = validate_price_with_grok(
        {
            "title": f"{brand} {model}",
    ...
    if bool(precision.get("is_suspicious")):
        logger.warning(
            "[Tier1 Precision] suspicious %s %s source=%s price=%.2f conf=%s risk=%s",
            brand,
            model,
            source,
            price_v,
...
class Sport2000Scraper:
    ...
    def scrape_model(self, brand: str, model: str) -> list[dict]:
        ...
        # Playwright fallback supprimé — bloque ThreadPoolExecutor sans timeout garanti.
        return _scrape_search_urls(self.SOURCE_NAME, brand, model, urls)
