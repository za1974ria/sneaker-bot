...
def _robust_price_aggregation(
    ...
    if len(prices_final) == 1:
        print("[WARNING] Only one valid price used")
...
def _fr_one_item_impl(
    ...
    for site_name, prices in pairs:
        for _p in prices or []:
            print("[DEBUG SCRAPER]", site_name, _p)
        if prices:
        ...
        try:
            ...
            if filtered:
                ...
                for _ep in by_site[site_name]:
                    print("[DEBUG SCRAPER]", site_name + "|post_filter", _ep)
        except Exception as e:  # noqa: BLE001
            logger.debug(
    ...
    try:
        ...
        if sitemap_prices:
            for _sx in sitemap_prices:
                print("[DEBUG SCRAPER]", "sitemap", float(_sx))
            logger.info(
...
def run_market(market: str, *, preserve_sources_csv: bool = False) -> None:
    ...
    with lock:
        ...
        for item in models:
            ...
            print(f"[DEBUG PRICES] {brand} {model_name} → {prices}")
        ...
        print("[DEBUG] sources_csv_rows:", len(source_rows))
        print("[DEBUG] sources_sample:", source_rows[:5])
        ...
        print("[DEBUG] BEFORE DEDUPE:", len(strict_sources))
        ...
        if not preserve_sources_csv or len(source_rows) >= max(existing_sources_count, 50):
        ...
        logger.info(
            "[run_summary] market=%s prix_bruts_total=%d lignes_sources_csv=%d scrapers_ok=%d scrapers_fail=%d",
            market_norm,
            prices_total,
            len(source_rows),
            src_ok,
        ...
        print(f"[SUMMARY] {src_ok}/{max(1, len(site_product_counts))} sources OK")
