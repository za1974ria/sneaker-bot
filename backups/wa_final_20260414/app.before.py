...
_ADMIN_TOPBAR = (
    '<div style="background:#0f1a13;border-bottom:1px solid #14532d;padding:8px 20px;'
    'display:flex;gap:12px;align-items:center;font-size:.8rem;font-family:system-ui">'
    '<span style="color:#4dff9f;font-weight:800;margin-right:8px">⚡ ADMIN</span>'
    '<a href="/admin" style="color:#9ca3af;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #1f2937">🏠 Dashboard</a>'
    '<a href="/comparison" style="color:#9ca3af;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #1f2937">🔍 Comparateur</a>'
    '<a href="/analytics" style="color:#9ca3af;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #1f2937">📊 Analytics</a>'
    '<a href="/supervisor" style="color:#9ca3af;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #1f2937">🤖 AEGIS</a>'
    '<a href="/admin/subscriptions" style="color:#9ca3af;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #1f2937">👥 Abonnements</a>'
    '<a href="/logout" style="color:#ef4444;text-decoration:none;padding:4px 10px;border-radius:6px;border:1px solid #3f1515;margin-left:auto">Déconnexion</a>'
    '</div>'
)
...
def remove_outliers(prices: list[float]) -> list[float]:
    ...
    print(f"[FILTER V2] disp={dispersion:.2f} kept={len(filtered)}")
    ...
    return JSONResponse(
        {
            ...
            "price_quality": _price_quality_snapshot(),
        }
    )
...
def robots() -> PlainTextResponse:
    return PlainTextResponse(
        ...
        "Disallow: /login\n"
        "Disallow: /api/\n"
        "Sitemap: https://sneakerbot.shop/sitemap.xml\n",
...
def sitemap() -> Response:
    path = _project_root() / "static" / "sitemap.xml"
...
<head>
  ...
  <title>Admin — SneakerBot</title>
  <style>
    *{box-sizing:border-box}
    body{margin:0;background:#0b0b0b;color:#e5e7eb;font-family:ui-sans-serif,system-ui,Arial,sans-serif;padding:24px}
    .wrap{max-width:900px;margin:0 auto}
    h1{margin:0 0 4px;font-size:1.55rem;color:#4dff9f;letter-spacing:.02em}
    .sub{color:#6b7280;margin:0 0 28px;font-size:.92rem}
    .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:14px}
    .card{background:#101010;border:1px solid #1f2937;border-radius:14px;padding:20px 22px;text-decoration:none;color:inherit;transition:border-color .15s,background .15s;display:block}
    .card:hover{border-color:#4dff9f;background:#0f1a13}
    .card-icon{font-size:1.8rem;margin-bottom:8px}
    .card-title{font-size:1rem;font-weight:800;color:#e5e7eb;margin:0 0 4px}
    .card-desc{font-size:.8rem;color:#6b7280;margin:0}
    .badge-admin{display:inline-block;background:#14532d;color:#4dff9f;border:1px solid #166534;border-radius:999px;font-size:.68rem;font-weight:700;padding:2px 9px;margin-left:8px;vertical-align:middle;text-transform:uppercase;letter-spacing:.05em}
    .topbar{display:flex;align-items:center;justify-content:space-between;margin-bottom:24px}
    .logout{color:#9ca3af;text-decoration:none;border:1px solid #2a2a2a;border-radius:8px;padding:5px 10px;font-size:.78rem;background:#111827}
    .logout:hover{border-color:#4dff9f;color:#4dff9f}
    .section-title{font-size:.72rem;font-weight:700;color:#4b5563;text-transform:uppercase;letter-spacing:.08em;margin:24px 0 10px}
  </style>
</head>
<body>
<div class="wrap">
  <div class="topbar">
    <div>
      <h1>SneakerBot <span class="badge-admin">Admin</span></h1>
      <p class="sub">Panneau d'administration — sneakerbot.shop</p>
    </div>
    <a href="https://sneakerbot.shop" class="logout" style="margin-right:8px">← Accueil</a>
    <a href="/logout" class="logout">Déconnexion</a>
  </div>

  <div class="section-title">Outils principaux</div>
  <div class="grid">
    <a href="/comparison" class="card">
      <div class="card-icon">🔍</div>
      <div class="card-title">Comparateur FR</div>
      <div class="card-desc">Comparaison multi-boutiques des prix sneakers en France</div>
    </a>
    <a href="/admin/subscriptions" class="card">
      <div class="card-icon">👥</div>
      <div class="card-title">Abonnements clients</div>
      <div class="card-desc">Gérer, valider et rejeter les demandes d'abonnement</div>
    </a>
    <a href="/analytics" class="card">
      <div class="card-icon">📊</div>
      <div class="card-title">Analytics</div>
      <div class="card-desc">Statistiques scraping, couverture et conversion</div>
    </a>
    <a href="/supervisor" class="card">
      <div class="card-icon">🤖</div>
      <div class="card-title">Superviseur IA (AEGIS)</div>
      <div class="card-desc">Lancement et suivi du pipeline de vérification IA</div>
    </a>
  </div>
  ...
  <div class="grid">
    <a href="/health" class="card">
      ...
      <div class="card-desc">État du service, fraîcheur des données, qualité des prix</div>
    </a>
    <a href="/api/admin/claude-validate?brand=Nike&limit=20" class="card" target="_blank">
      <div class="card-icon">🧠</div>
      <div class="card-title">Validation Claude AI</div>
      <div class="card-desc">Analyse anomalies de prix par Claude Haiku — détection boutiques suspectes</div>
    </a>
  </div>
</div>
</body>
...
def _claude_validate_price_batch(
    ...
    *,
    ...
    if not api_key or not rows:
        print(f"[SMART SCORE] computed={smart_score} prices={len(prices)}")
        return {
            "score": smart_score,
            "anomalies": [],
    ...
    try:
        print("[CLAUDE] START validation")
        brand = str(prices[0].get("brand", "?")) if prices else "?"
        print(f"[CLAUDE] prices_count={len(prices)} brand={brand}")
        ...
        print("[CLAUDE RESPONSE RAW]", response)
        ...
        traceback.print_exc()
        logger.warning("_claude_validate_price_batch: %s", e)
        print(f"[SMART SCORE] computed={smart_score} prices={len(prices)}")
...
def api_comparison_fr(
    ...
    for _mk, _shop_items in _shops_by_model.items():
        ...
        _agg["source_count"] = _sc
        _agg["sources_preview"] = []  # Confidentiel
        _agg["price_confidence"] = "high" if _sc >= 3 else ("medium" if _sc == 2 else ("low" if _sc == 1 else "none"))
    ...
    try:
        ...
        with _sqlite3.connect(str(_PH_DB_PATH), timeout=3) as _phconn:
            _ph_rows = _phconn.execute("SELECT DISTINCT brand, model FROM price_history").fetchall()
            _models_with_history = {(r[0].strip().lower(), r[1].strip().lower()) for r in _ph_rows}
    except Exception:
        _models_with_history = set()
    for item in items:
...
def api_scorecard_fr(
    ...
    if not items:
        ...
        _used_fallback = True
    if not items:
        # Vraiment aucune donnée pour ce filtre.
        logger.warning(
...
def comparison_page(request: Request, brand: str = "", model: str = "") -> HTMLResponse:
    ...
    admin_topbar = _ADMIN_TOPBAR if _is_admin_session(request) else ""
    html = f"""<!doctype html>
<html lang="fr">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    .cmp-market-state {{display:block;margin-top:4px;font-size:.72rem;font-weight:700;line-height:1.25}}
    .cmp-market-state.stable {{color:#22c55e}}
    .cmp-market-state.variable {{color:#f59e0b}}
    .cmp-market-state.unstable {{color:#ef4444}}
    .cmp-filter-note {{display:block;margin-top:3px;font-size:.68rem;color:#9ca3af;line-height:1.25}}
    .market-observation {{
      font-size:.75rem;
      color:#9ca3af;
      text-align:right;
    ...
    .market-observation.stable {{color:#4ade80}}
    .market-observation.variable {{color:#facc15}}
    .market-observation.unstable {{color:#f87171}}
    tr.cmp-observation-row td {{
      padding-top:2px;
      padding-bottom:8px;
    ...
    .btn-cmp-wa{{
      display:inline-flex;align-items:center;gap:4px;background:#075e54;color:#25d366;
      border:1px solid #128c7e;border-radius:8px;padding:6px 11px;font-size:.78rem;
      font-weight:800;text-decoration:none;white-space:nowrap;
    }}
    .btn-cmp-wa:hover{{filter:brightness(1.15);border-color:#25d366}}
    ...
    .src-badge{{display:inline-flex;align-items:center;gap:0.3rem;padding:0.2rem 0.55rem;border-radius:999px;font-size:.73rem;font-weight:700;cursor:pointer;border:1px solid transparent;white-space:nowrap;transition:opacity .15s}}
    .src-badge:hover{{opacity:.8}}
    .sb-high{{background:rgba(34,197,94,.15);color:#22c55e;border-color:rgba(34,197,94,.3)}}
    .sb-medium{{background:rgba(249,115,22,.15);color:#f97316;border-color:rgba(249,115,22,.3)}}
    .sb-low{{background:rgba(239,68,68,.15);color:#ef4444;border-color:rgba(239,68,68,.3)}}
    .sb-none{{background:rgba(107,114,128,.15);color:#6b7280;border-color:rgba(107,114,128,.3)}}
    #src-modal-overlay{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999;align-items:center;justify-content:center}}
    #src-modal-overlay.open{{display:flex}}
    #src-modal{{background:#111827;border:1px solid #1f2937;border-radius:14px;padding:24px 28px;max-width:400px;width:90%;color:#e5e7eb}}
    #src-modal h4{{margin:0 0 14px;color:#00ff88;font-size:1rem}}
    #src-modal ul{{margin:0;padding:0 0 0 18px;font-size:.9rem;line-height:1.8}}
    #src-modal-close{{margin-top:16px;background:#0f2f1f;color:#4dff9f;border:1px solid #166534;border-radius:8px;padding:6px 14px;font-size:.8rem;cursor:pointer}}
    th:nth-child(5),td:nth-child(5),
    th:nth-child(6),td:nth-child(6),
    th:nth-child(7),td:nth-child(7),
    th:nth-child(9),td:nth-child(9){{display:none !important}}
    @media (max-width: 375px) {{.src-badge{{font-size:.7rem;padding:.18rem .4rem}}}}
...
{admin_topbar}
  ...
  <main class="wrap">
    ...
    <div id="product-info">
      <img id="product-image" style="width:120px;" alt="">
      <div id="product-name"></div>
      <div id="product-ref"></div>
    </div>
    <div class="table-wrap">
      <table id="cmp-table">
        <thead><tr><th>Produit</th><th class="min">Min</th><th class="moyen">Moyen</th><th class="max">Max</th><th>Score</th><th>Crédibilité</th><th>Google</th><th>Recommandation</th><th>Position</th><th>Sources</th><th>Historique</th></tr></thead>
  ...
  <script>
    ...
    function renderRows(arr, relaxed=false) {{
      ...
      rows.innerHTML = note + arr.map(it => {{
        ...
        const filterHint = `<span class="cmp-filter-note">Analyse basée sur ${{Number.isFinite(sourcesCount) ? sourcesCount : 0}} sources (valeurs extrêmes filtrées)</span>`;
        ...
        const priceCells = `<td class="cmp-price"><span class="cmp-price-inner">${{safePrices.safeMin.toFixed(2)}}${{String.fromCharCode(0xA0)}}€</span></td>
          <td class="cmp-price"><span class="cmp-price-inner">${{safePrices.safeAvg.toFixed(2)}}${{String.fromCharCode(0xA0)}}€</span></td>
          <td class="cmp-price"><span class="cmp-price-inner">${{safePrices.safeMax.toFixed(2)}}${{String.fromCharCode(0xA0)}}€</span></td>`;
        return `<tr>
  ...
  <div id="src-modal-overlay">
    <div id="src-modal">
      <h4>Sources de prix</h4>
      <div id="src-modal-body"></div>
      <button id="src-modal-close" onclick="document.getElementById('src-modal-overlay').classList.remove('open')">Fermer</button>
    </div>
  </div>
...
<body>
  ...
  <script>
    ...
    html = html.replace("<body>", "<body>" + admin_topbar, 1)
    return HTMLResponse(content=html)
